...
37. E8 本地落地后，系统管理员配置口径已从旧 `TOKENINSIDE_ADMIN_OPEN_IDS` 收口到新 `TOKENINSIDE_SYSTEM_ADMIN_OPEN_IDS`，旧变量仍兼容读取。后续部署时生产 `.env.production` 应优先填写新变量，至少配置 1 个系统管理员 open_id，否则部门主管解析失败时仍无法兜底发送审批卡片。
38. 系统管理员兜底审批的用户提示当前只在申请/额度重置接口响应中返回，并由用户后台 message/alert 展示；真实体验验收需要覆盖“用户无组织或主管不可识别”的真实飞书用户，确认卡片实际发送到系统管理员且前端展示固定提示文案。
39. 管理员查看/指派能力已本地新增 API 和 UI，但当前指派目标要求该 open_id 已存在于 TokenInside 用户表。首次给从未登录过 TokenInside 的飞书用户指派管理员，仍需要对方先完成一次飞书 OAuth 登录，或后续补按 open_id 拉取飞书用户资料再建档。
40. 用户后台剩余额度当前以 NewAPI `remain_quota` 为权威来源，按 `NEWAPI_QUOTA_PER_UNIT` 换算展示；当 NewAPI 查询失败时不会用 token usage 简单相减伪造剩余额度，而是保留未展示/回退状态，避免单位错误。
41. `.env.example` 和 `.env.production.example` 已改为中文变量说明，并补充“变量干什么、在哪里获取”。真实密钥仍不得写入仓库示例文件；生产部署应只把真实值写入服务器私有 `.env.production` 或容器环境变量。
42. token/额度大数展示已参考 Aether 口径收敛为十进制 K/M/B/T：1000 以下原样，1000 以上按 2/1/0 位小数自动缩写。该格式仅用于只读展示；审批额度、调额和默认额度配置输入框继续使用原始数字，避免用户输入 `1.2M` 这类缩写造成解析歧义。
43. 最新 UI 反馈说明上一批用户后台/管理后台收口仍存在细节残留：PC 用户卡片右侧“管理后台”入口与右侧三个组件未在同一视觉轴线，需要垂直居中对齐。
44. 移动端管理后台页的同类组件和刷新按钮不应漂浮在不一致的位置；目标是放到右下方，并与左侧“管理范围”下方文字形成整洁的视觉对齐。
45. 管理范围不再需要独立卡片展示；后续实现应把管理范围收敛到用户身份区、页面头部或紧凑标签，减少管理后台首屏重复容器。
46. 管理后台上方数据卡当前长条形占用空间过多；下一步应改为正方形紧凑指标块，并检查桌面/移动端都不发生文字挤压或重叠。
