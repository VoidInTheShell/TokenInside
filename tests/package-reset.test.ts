...
test("a mid-month reset changes periods exactly at Hong Kong midnight", () => {
  ...
  assert.equal(packageBillingPeriod(policy, before), "2026-07");
  assert.equal(packageBillingPeriod(policy, atReset), "2026-08");
...
test("day 31 clamps to the final Hong Kong day in short months", () => {
  ...
  assert.equal(packageBillingPeriod(policy, beforeFebruaryReset), "2027-02");
...
test("day one retains calendar-month identifiers", () => {
  ...
  assert.equal(
    packageBillingPeriod(policy, new Date("2026-07-31T15:59:59.999Z")),
    "2026-07",
  );
});
