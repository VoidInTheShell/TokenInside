...
const adminSelfProvisionPath = new URL(
  "../app/api/token/admin-provision/route.ts",
  import.meta.url,
);
...
test("管理员用户后台用幂等 POST 补发 Key 且 session GET 继续只读", async () => {
  const [route, client, session] = await Promise.all([
    readFile(adminSelfProvisionPath, "utf8"),
    readFile(experienceClientPath, "utf8"),
    readFile(sessionPath, "utf8"),
  ]);

  assert.match(route, /export async function POST\(\)/);
  assert.match(route, /const auth = await requireAdminScope\(\)/);
  assert.match(route, /await ensureAdminDefaultProvisioning\(\{/);
  assert.match(route, /feishuUserId:\s*auth\.user\.id/);
  assert.match(route, /trustedScope:\s*auth\.scope/);
  assert.doesNotMatch(route, /export async function GET/);
  assert.match(client, /fetch\("\/api\/token\/admin-provision", \{ method: "POST" \}\)/);
  assert.match(client, /adminProvisioningAttemptRef\.current === userId/);
  assert.match(client, /await refresh\(\{ quiet: true \}\)/);
  assert.doesNotMatch(session, /admin-provision|ensureAdminDefaultProvisioning/);
});

test("管理员首次登录持续显示等待界面直到 active Key 回读", async () => {
  const source = await readFile(experienceClientPath, "utf8");
  assert.match(source, /const adminFirstLoginPending = adminFirstLoginNeedsProvisioning\(/);
  assert.match(source, /const shouldAutoRefreshSession = Boolean\([\s\S]*adminFirstLoginPending/);
  assert.match(source, /const loginInProgress =[\s\S]*adminFirstLoginPending/);
  assert.match(
    source,
    /mode=\{adminFirstLoginPending \? "admin-provisioning" : "authentication"\}/,
  );
  assert.match(source, /error=\{adminFirstLoginPending \? adminProvisioningError : null\}/);
  assert.match(source, /onRetry=\{adminFirstLoginPending \? retryAdminProvisioning : undefined\}/);
  assert.doesNotMatch(source, /管理员用户后台已经开放/);
});
