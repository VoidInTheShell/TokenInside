...
| P1 | complete_local | 管理员治理与用户后台收口：系统管理员可查看全部管理员、指派管理员；用户后台补当前账期剩余额度，调整 active key 提示、刷新按钮、自动识别动画、管理入口、最新审批请求显示位置、品牌副标题和模型列表说明文案 |
...
13. P2：审批闭环、数据面复测、E8 近期优先修复包部署验收完成后，再继续管理员取消用户权限/禁用 active key、部门归属历史快照、真实月度账期重置和部门报表。
