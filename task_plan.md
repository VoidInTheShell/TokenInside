...
| P1 | in_progress | E9 管理后台重做立即落地：将“额度管理”“管理员”“额度统计”合并为“用户管理”；新增系统管理员专用“部门统计”；部门管理员只可查看本部门下属用户的“用户统计”；将 Aether 风格“使用记录”迁入用户后台和管理后台，并补齐禁用/删除后重新申请链路 |
...
14. E9-1：立即重做管理后台信息架构：旧“额度管理”“管理员”“额度统计”合并为“用户管理”，默认用表单/表格查看系统所有授权范围内用户；操作列提供调整额度、指派管理员、禁用和删除。
15. E9-2：立即补后台服务端范围裁剪：系统管理员可看全部；部门管理员的用户管理、用户统计和使用记录只能覆盖该部门下属用户；部门管理员不得查看部门统计。
16. E9-3：立即补系统管理员专用“部门统计”：按部门展示成员数、已发放 key 用户数、当前账期发放额度、剩余额度、已用 tokens、请求数、最近调用和占比。
17. E9-4：立即迁移 Aether 风格“使用记录”：用户后台只看本人记录，管理后台按系统/部门管理范围查看记录；支持按用户、部门、状态、模型、接口和时间过滤。
18. E9-5：立即补禁用/删除语义：禁用 active key 后代理不可用但保留历史；删除为软删除/撤销资格，禁用 active key 并让用户重新进入申请流程，历史用量和审计不得删除。
19. P2：E9 首轮落地和验证完成后，再继续更完整的部门历史快照、NewAPI logs 校准、复杂报表和生产部署。
