...
| P1 | complete_e9_10_local | E9-10 部门名称展示修复已本地落地：保留 `departmentId` 作为权限/过滤主键，同时通过飞书部门详情接口补齐 `departmentName`；登录回调、`/api/session` 懒同步、管理端用户列表、用户统计、部门统计、用量筛选、usage records 和 `/v1` 新 proxy log 快照均已贯通部门名称展示。前端统一按 `departmentName -> departmentId 脱敏` 兜底；本地 `npm run typecheck` 与 `npm run build` 通过，尚未构建镜像或部署 LA。 |
...
23. E9-10：已本地落地部门名称展示修复。飞书 `department_ids` 继续作为稳定部门 ID；TokenInside 在登录回调、`hydrateUserDepartment()` 懒同步、用户/管理 API 响应和 `/v1` 新 proxy log 写入时补齐 `departmentName`。用户卡片、管理端用户列表、管理员范围、部门统计、用户统计、用量筛选和 usage records 均优先展示部门名称，缺失时才展示脱敏 departmentId。权限和过滤仍使用稳定 `departmentId`，不能把部门名称作为权限主键；本轮未部署公网实例。
24. E9-12：管理员撤权与自动恢复阻断已本地落地。root 删除任意用户时会撤销其非环境管理员范围，用户重新进入普通申请/审批链路；root 取消或删除部门/系统管理员时，其管理员入口和管理 API 权限都会被收回。手动取消、用户删除和历史禁用的管理员范围会阻断部门主管自动识别重新授予；只有 root 后续显式重新指派可恢复管理员资格。环境变量 root 仍为只读来源。本地 `npm run typecheck`、`npm run build` 和 `git diff --check` 均通过；本轮未部署公网实例。
25. E5R：基于当前代码重新评估后，E5 不再按“从零补用量统计”执行，改为 NewAPI 用量同步生产化：新增 `newapi_usage_records`、`usage_sync_checkpoints`、`usage_sync_issues`，补 checkpoint + overlap 增量同步、usage records 与 proxy logs 匹配回填、账期聚合 source record 化、管理端同步状态和问题追踪。自动同步周期配置和 active key `remain_quota` 校准写回迁入 F 阶段。
26. P2/F：E9 首轮、E9-8、E9-9、E9-10、E9-12 和 E5R 之后，F 阶段再处理用量同步周期配置/自动执行、active key `remain_quota` 校准写回，以及更完整的部门历史快照、详情抽屉深度追踪、真实月度重置和生产部署策略收口。
